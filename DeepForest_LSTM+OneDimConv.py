...
if __name__ == "__main__":
    ...
    for i in range(1, 11):
        for k in range(1, 2):
            ...
            a_train0 = np.load('trainmat_onedimconv'+str(i)+'.npy')
            a_test0 = np.load('testmat_onedimconv'+str(i)+'.npy')
            print((a_train0.shape, a_test0.shape))
            a_train1 = np.load('trainmat_lstm'+str(i)+'.npy')
            a_test1 = np.load('testmat_lstm'+str(i)+'.npy')
            ...
            a_train2 = uni[0:a_train.shape[0], :]
            a_test2 = uni[a_train.shape[0]:(a_test.shape[0] + a_train.shape[0]), :]
            a_train_uni = np.concatenate((a_train1, a_train2), axis=1)
            a_test_uni = np.concatenate((a_test1, a_test2), axis=1)
            print((a_train2.shape, a_test2.shape))
            a_train_uni = np.concatenate((a_train2, a_train1), axis = 1)
            a_test_uni = np.concatenate((a_test2, a_test1), axis=1)
            # print((a_train_uni.shape, a_test_uni.shape))
            # a_test3 = preprocessing.Normalizer().fit_transform(a_test3)
            # a_train3 = preprocessing.Normalizer().fit_transform(a_train3)
            # test(a_train_uni, a_test_uni, b_train, b_test, i)
            # print('........')
            # print('3->2')
            # print('........')
            test(a_train1, a_test1, b_train, b_test, i+10)