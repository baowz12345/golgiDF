...
def test(A_train, A_test, B_train, B_test, k):
    ...
    for x in range(45, 50, 2):
        for y in range(90, 111, 2):
            for z in range(10, A_test.shape[1]+1, 3):
    # for x in range(45, 46, 2):
    #     for y in range(90, 91, 2):
    #         for z in range(10, 15, 3):
    ...
    np.savetxt('evaluate_record_lstm' + str(k) + '.csv', re, delimiter=',',fmt = '%s')
    np.savetxt('result_auc' + str(k) + '.csv', auc_, delimiter=',')
...
if __name__ == "__main__":
    ...
    for ii in range(1, 11):
        for k in range(1, 2):
            ...
            uni_pre = np.concatenate((a_train, a_test), axis=0)
            a_train0 = np.load('trainmat_onedimconv'+str(ii)+'.npy')
            a_test0 = np.load('testmat_onedimconv'+str(ii)+'.npy')
            print((a_train0.shape, a_test0.shape))
            a_train1 = np.load('trainmat_lstm'+str(ii)+'.npy')
            a_test1 = np.load('testmat_lstm'+str(ii)+'.npy')
            a_train1 = np.concatenate((a_train0, a_train1), axis=1)
            a_test1 = np.concatenate((a_test0, a_test1), axis=1)
            a_train1 = preprocessing.Normalizer().fit_transform(a_train1)
            a_test1 = preprocessing.Normalizer().fit_transform(a_test1)
            print((a_train1.shape, a_test1.shape))

            ridge = Ridge().fit(a_train, b_train)
            reg_coef = ridge.coef_.tolist()
            rid = []
            for i in range(32):
                rid.append(uni_pre[:, reg_coef.index(max(reg_coef))])
                reg_coef[reg_coef.index(max(reg_coef))] = -1000
            rid = np.array(rid)
            rid = np.transpose(rid)
            rid = preprocessing.Normalizer().fit_transform(rid)
            a_train2 = rid[0:a_train.shape[0], :]
            a_test2 = rid[a_train.shape[0]:(a_test.shape[0] + a_train.shape[0]), :]

            phateo = phate.PHATE(n_components=32)
            # pca = PCA(n_components=32, svd_solver="auto", whiten=True)
            pha = phateo.fit_transform(uni_pre)
            pha = preprocessing.Normalizer().fit_transform(pha)
            a_train3 = pha[0:a_train.shape[0], :]
            a_test3 = pha[a_train.shape[0]:(a_test.shape[0] + a_train.shape[0]), :]

            a_train_uni1 = np.concatenate((a_train1, a_train2), axis=1)
            a_test_uni1 = np.concatenate((a_test1, a_test2), axis=1)
            print((a_train_uni1.shape, a_test_uni1.shape))
            a_train_uni2 = np.concatenate((a_train1, a_train3), axis = 1)
            a_test_uni2 = np.concatenate((a_test1, a_test3), axis=1)
            print((a_train_uni2.shape, a_test_uni2.shape))
            a_train_uni3 = np.concatenate((a_train_uni1, a_train3), axis=1)
            a_test_uni3 = np.concatenate((a_test_uni1, a_test3), axis=1)
            print((a_train_uni3.shape, a_test_uni3.shape))
            test(a_train_uni1, a_test_uni1, b_train, b_test, ii)
            # print('........')
            # print('ridge->phate')
            # print('........')
            # test(a_train_uni2, a_test_uni2, b_train, b_test, ii+10)
            # print('........')
            print('3->4')
            # print('........')
            test(a_train_uni3, a_test_uni3, b_train, b_test, ii+20)