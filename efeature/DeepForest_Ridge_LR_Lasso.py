...
if __name__ == "__main__":
    ...
    for i in range(1, 2):
        for k in range(1, 11):
            ...
            uni = []
            for i in range(32):
                uni.append(uni_pre[:, reg_coef.index(max(reg_coef))])
                reg_coef[reg_coef.index(max(reg_coef))] = -1000
            # pca = PCA(n_components=32, svd_solver="auto", whiten=True)
            uni = np.array(uni)
            uni = np.transpose(uni)

            lasso = Lasso().fit(a_train, b_train)
            reg_coef = lasso.coef_.tolist()
            uni2 = []
            for i in range(32):
                uni2.append(uni_pre[:, reg_coef.index(max(reg_coef))])
                reg_coef[reg_coef.index(max(reg_coef))] = -1000
            uni2 = np.array(uni2)
            uni2 = np.transpose(uni2)

            ridge = Ridge().fit(a_train, b_train)
            reg_coef = ridge.coef_.tolist()
            uni3 = []
            for i in range(32):
                uni3.append(uni_pre[:, reg_coef.index(max(reg_coef))])
                reg_coef[reg_coef.index(max(reg_coef))] = -1000
            uni3 = np.array(uni3)
            uni3 = np.transpose(uni3)

            a_train1 = uni[0:a_train.shape[0], :]
            a_test1 = uni[a_train.shape[0]:(a_test.shape[0]+a_train.shape[0]), :]
            a_train2 = uni2[0:a_train.shape[0], :]
            a_test2 = uni2[a_train.shape[0]:(a_test.shape[0] + a_train.shape[0]), :]
            a_train3 = uni3[0:a_train.shape[0], :]
            a_test3 = uni3[a_train.shape[0]:(a_test.shape[0] + a_train.shape[0]), :]
            print(k)
            print(a_train1.shape)
            print(a_train2.shape)
            print(a_train3.shape)
            # test(a_train, a_test, b_train, b_test, k)
            test(a_train1, a_test1, b_train, b_test, k)# RL
            test(a_train2, a_test2, b_train, b_test, k+10)# lasso
            test(a_train3, a_test3, b_train, b_test, k+20)  # ridge